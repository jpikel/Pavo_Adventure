...
class Game:
	...
	def load_from_file(self):
		...
		self.player.cold = p["cold"]
		self.player.hunger =["hunger"]
		self.player.illness = p["illness"]
		self.player.name = p["name"]
		self.player.rescued = p["rescued"]
		self.player.dead = p["dead"]
		self.player.inventory= p["inventory"]

		#print r
		#load room info from saved game
		#self.current_room.id = r["id"]
		#self.current_room.title = r["title"]
		#self.current_room.visited = r["visited"]
		#self.current_room.long_description = r["long_description"]
		#self.current_room.connected_rooms = r["connected_rooms"]
		#self.current_room.items = r["items"]
		self.current_room = files.load_room(r["title"])
	...
	def gameCycle(self):
		...
		print "Current Room: ", self.current_room['title']
		...
		while True:
			...
			if userInput == "savegame":
				print "Are you sure you wish to save y/n"
				checkYes = raw_input("->")
				str.lower(checkYes)
				if checkYes == "y":
			...
			#load
				...
				print "Loading will exit game.  Are you sure you wish to load? y/n"
				checkYes = raw_input("->")
				str.lower(checkYes)
				if checkYes == "y":
					self.load_from_file()
			...
			#quit
				...
				if saved ==False:
					...
					if checkYes == "y":
						...
						print "Do you wish to save and quit? y/n"
						checkYes = raw_input("->")
						if checkYes == "y":
							files.save_game(self.player, self.current_room)
							self.exitGame()
