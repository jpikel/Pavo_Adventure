...
class Game():
	...
	def post_process(self, res):
		...
		if not self.player.get_death_status() and not self.player.get_rescue_status():
		   ...
		   if USE_CURSES: 
				#game_ui.write_main(msg)
				
				helpers.REAPER.append(msg)
				game_ui.write_main_artifact(helpers.REAPER)
	...
	def room_action(self, title_or_compass, action):
		...
		if action == "go":
			...
			res.action = 'look'
			res.description = self.room.long_desc
			if (title_or_compass != self.room.current_room):
				res.description = "you are not in that room \n \n" + res.description
		else:
